...
export function PlantillaMinimalista({ datos, perfilGithub, reposGithub }: PlantillaProps) {
  ...
  return (
    ...
    >
      ...
      {education.length > 0 && (
        <section style={{ marginBottom: '32px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {education.map((edu) => (
              <div key={edu.id} style={{ display: 'flex', justifyContent: 'space-between', breakInside: 'avoid' }}>
                <div>
      ...
      <div style={{ display: 'flex', gap: '48px', breakInside: 'avoid', flexWrap: 'wrap' }}>
        {skills.length > 0 && (
      ...
      {/* GitHub */}
      {perfilGithub && (
        <section style={{ marginTop: '32px', paddingTop: '24px', borderTop: '1px solid #e5e7eb', breakInside: 'avoid' }}>
          <h2 style={{ fontSize: '11px', fontWeight: '600', color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '2px', marginBottom: '12px' }}>
            GitHub
          </h2>
